...
(function () {
  const __VUE__ = document.querySelector(".infinite-craft").__vue__;
          ...
          for (const result of allResults) {
            ...
            if (encounteredElements.has(result)) {
              if (encounteredElements.get(result)![0]!.length - 1 === depth) {
                encounteredElements
                  .get(result)!
                  .push([...seedAsArray, result].sort());
              }
            } else {
              encounteredElements.set(result, [
                [...seedAsArray, result].sort(),
              ]);
  ...
  function addElementToStorage(elementText: string) {
    if (
      ...
      unsafeWindow.IC.getItems().filter((e) => e.text === elementText)
        .length !== 0
    ) {
      return;
    }
